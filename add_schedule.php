...
// echo "Instructor ID: " . (isset($_SESSION['instructor_id']) ? $_SESSION['instructor_id'] : 'Not Set') . "<br>";

// Обработка удаления временного слота
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_time_window'])) {
    if (isset($_SESSION['instructor_id'])) {
        $instructor_id = $_SESSION['instructor_id'];
        $time_window_id = $_POST['time_window_id'];

        try {
            deleteTimeWindow($time_window_id);
            header('location: schedule.php');
            exit();
        } catch (Exception $e) {
            $errMsg = "Ошибка при удалении временного слота: " . $e->getMessage();
        }
    } else {
        $errMsg = "Ошибка: Инструктор не найден.";
    }
}

// Обработка добавления временного слота
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_time_window'])) {
    if (isset($_SESSION['instructor_id'])) {
        $instructor_id = $_SESSION['instructor_id'];
        $start_time = trim($_POST['start_time']);
        $end_time = trim($_POST['end_time']);
        $max_slots = isset($_POST['max_slots']) ? (int)$_POST['max_slots'] : 1;

        if ($start_time === '' || $end_time === '') {
            $errMsg = "Заполните все поля!";
        } else {
            try {
                addTimeWindow($instructor_id, $start_time, $end_time, $max_slots);
                header('location: schedule.php');
                exit();
            } catch (Exception $e) {
                $errMsg = "Ошибка при добавлении временного слота: " . $e->getMessage();
            }
        }
    } else {
        $errMsg = "Ошибка: Инструктор не найден.";
...
$instructor_id = $_SESSION['instructor_id'];
$time_windows = getTimeWindows($instructor_id);
?>
...
<body>
        ...
        <main class="main">
            <div class="content">
                ...
                <form method="post">
                    ...
                    <label for="max_slots">Количество свободных слотов:</label>
                    <input type="number" name="max_slots" id="max_slots" value="1" min="1" required> <br>
                    <button type="submit" name="add_time_window">Добавить</button>
                </form>
                <?php if ($errMsg): ?>
                    <p><?= $errMsg ?></p>
                <?php endif; ?>
                ...
                <table>
                    ...
                    <?php foreach ($time_windows as $window): ?>
                        <tr>
                            <td><?= strftime('%m-%d %H:%M', strtotime($window['start_time'])) ?></td>
                            <td><?= strftime('%H:%M', strtotime($window['end_time'])) ?></td>
                            <td>
                                <form method="post" style="display: inline;">
                                    <input type="hidden" name="time_window_id" value="<?= $window['time_window_id'] ?>">
                                    <button type="submit" name="delete_time_window">Отменить</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <script src="js/log_of.js"></script>
