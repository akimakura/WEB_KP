...
// Получение информации о бронированиях для текущего инструктора
$bookings = $pdo->prepare("
    SELECT b.bookings_id, b.start_time, b.end_time, u.name AS user_name, u.surname AS user_surname, k.name AS kid_name, k.surname AS kid_surname, s.name AS service_name
    FROM bookings b
    ...
    <form class="wrapper" action="log_of.php" method="post">
        ...
        <main class="main">
            <div class="content">
                ...
                <h2>Доступные временные интервалы</h2>
        ...
        <table>
            ...
            <?php foreach ($time_windows as $window): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($window['max_slots']) ?></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
    ...
    <div class="scrollable-table">
        <table>
            ...
            <?php foreach ($bookings as $booking): ?>
                <tr>
                    <td><?= strftime('%m-%d %H:%M', strtotime($booking['start_time'])) ?></td>
                    <td><?= strftime('%H:%M', strtotime($booking['end_time'])) ?></td>
                    <td><?= htmlspecialchars($booking['kid_name'] . ' ' . $booking['kid_surname']) ?></td>
